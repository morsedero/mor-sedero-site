...
for(const scheme of ["dark","light"]){
 ...
 const auto=await p.evaluate(()=>({
   panelsOpen:document.querySelectorAll(".details").length,
   checkboxes:document.querySelectorAll(".details li .box").length,
   expandBtns:document.querySelectorAll(".expand").length,
   accent:getComputedStyle(document.documentElement).getPropertyValue("--accent").trim(),
   boardDots:[...document.querySelectorAll(".rows .row .dot")].map(d=>getComputedStyle(d).backgroundColor)
 }));
 if(scheme==="dark"){
   // toggle the first unchecked item
   const box=p.locator(".details li:not(.on) .box").first();
   const before=await p.evaluate(()=>[...document.querySelectorAll(".details li")].map(l=>l.classList.contains("on")?1:0));
   await box.click(); await p.waitForTimeout(800);
   const after=await p.evaluate(()=>({
     states:[...document.querySelectorAll(".details li")].map(l=>l.classList.contains("on")?1:0),
     counts:[...document.querySelectorAll(".details .clname span:last-child")].map(x=>x.textContent),
