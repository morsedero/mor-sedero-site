...
async function go(label, when, stub, navNext){
  ...
  const r=await p.evaluate(()=>{
    ...
    /* isBreak excluded: a Break is a task-shaped block with deep=false, so
       "not deep" alone means "quick task OR rest" — see hour.js's own note
       on the same trap. Left in here, the Break's span silently inflated
       the quick-task span check below once bricks made that check actually
       multiply by task count. */
    const blocks=allEvents().filter(e=>e.isTask&&!e.isBreak&&overlapsDay(e,d))
      .map(e=>({s:minsOf(e.start),e:minsOf(e.end),deep:e.deep,taskCount:e.cardShorts.length})).sort((a,b)=>a.s-b.s);
    return {day:d.getDay(),state:(document.querySelector(".state h2")||{}).textContent||null,
      blocks,quickTotal:CFG.quickTotal,creates:window.__calls.filter(c=>c.tool==="create_event").length};});
  const q=r.blocks.filter(x=>!x.deep);
  const span=q.length?q[q.length-1].e-q[0].s:0;
  /* Quick tasks now merge into one shared block (see CLAUDE.md's brick
     rules) — its span is n * quickTotal, n being the number of tasks
     inside it, not quickTotal alone. */
  const quickTaskCount=q.reduce((n,x)=>n+(x.taskCount||1),0);
  const bad=[];
  ...
  if(span>quickTaskCount*r.quickTotal) bad.push(`quick span ${span} > ${quickTaskCount}*${r.quickTotal}`);
  console.log(`[${label}] ${errs.length?"ERR "+errs.join("|"):"ok"} day=${r.day} state=${JSON.stringify(r.state)} `+
    `quickTotal=${r.quickTotal} span=${span} blocks=${r.blocks.length}`);
