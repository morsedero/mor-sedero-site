/* Friday is an ordinary planning day: no prompt, tasks scheduled by available time. */
...
(async()=>{
  ...
  for(const [label,when] of [["friday morning","2026-08-14T09:00:00+03:00"],
    ...
    if(label==="saturday"){
      if(r.state!=="My Day Off") bad.push("day-off state missing");
      if(r.quick||r.short||r.long) bad.push("scheduled something on the day off");
    }else{
      if(!r.hub) bad.push("no hub");
    }
    console.log(`[${label}] ${errs.length?"ERR "+errs.join("|"):"ok"} ${r.quick}q ${r.short}sh ${r.long}lg · prompt=${r.prompt}` +
                (bad.length?"\n  ✗ "+bad.join("\n  ✗ "):"  ✓"));
