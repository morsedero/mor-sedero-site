...
await p.locator("#pageMain .details .desc-row .edit-pencil").first().click();
await p.waitForTimeout(150);
const ta = p.locator("#pageMain .details .edit-row textarea").first();
await ta.fill("Edited description text");
await p.locator("#pageMain .details .edit-acts .btn.primary").first().click();
await p.waitForTimeout(400);
const afterDesc = await p.evaluate(()=>({
  text:(document.querySelector("#pageMain .details .desc")||{}).textContent||null,
  editorGone: !document.querySelector("#pageMain .details textarea"),
...
const firstItem = p.locator("#pageMain .details li").first();
...
await p.waitForTimeout(150);
const inp = p.locator("#pageMain .details li.editing .edit-item").first();
await inp.fill("should not stick");
await inp.press("Escape");
await p.waitForTimeout(300);
const afterCancel = await p.evaluate(()=>({
  editorGone: !document.querySelector("#pageMain .details .edit-item"),
...
await firstItem.locator(".edit-pencil").click();
await p.waitForTimeout(150);
const inp2 = p.locator("#pageMain .details li.editing .edit-item").first();
await inp2.fill("Renamed item text");
await inp2.press("Enter");
await p.waitForTimeout(400);
const afterSave = await p.evaluate(()=>({
  text:(document.querySelector("#pageMain .details li .it")||{}).textContent||null,
  editorGone: !document.querySelector("#pageMain .details .edit-item"),
  write:(window.__calls.find(c=>c.tool==="trelloWriteChecklist"&&c.input.action==="update_item"&&c.input.text)||{}).input||null,
  boxIntact:(document.querySelector("#pageMain .details li .box")||{}).getAttribute&&document.querySelector("#pageMain .details li .box").getAttribute("aria-pressed")
}));
